...
cursor.execute(
    """CREATE TABLE IF NOT EXISTS products_product (
                    ...
                    supplier_id BIGINT UNIQUE REFERENCES suppliers_supplier,
...
faker = Faker()
supplier_ids = set()  # 用来保存已经生成的 supplier_id
for _ in range(100):  # 生成 100 条测试数据
    ...
    # 生成唯一的 supplier_id
    supplier_id = faker.unique.random_number(digits=6)
    while supplier_id in supplier_ids:
        supplier_id = faker.unique.random_number(digits=6)
