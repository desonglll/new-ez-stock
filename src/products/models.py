...
class Product(models.Model):
    ...
    user = models.ForeignKey(
        User, default=1, null=True, on_delete=models.SET_NULL, verbose_name="谁可见"
    )
    ...
    sku = models.CharField(
        max_length=50,
        unique=True,
        default=generate_default_sku,
        verbose_name="SKU 编号",
    )
    image = models.CharField(
        max_length=255, blank=True, null=True, verbose_name="产品图片"
    )

    # 价格和销售信息
    price = models.DecimalField(
        blank=True, null=True, max_digits=10, decimal_places=2, verbose_name="价格"
    )
    discount_price = models.DecimalField(
        max_digits=10, decimal_places=2, blank=True, null=True, verbose_name="折扣价格"
    )
    ...
    attributes = models.ManyToManyField(
        "ProductAttribute", related_name="products", blank=True, verbose_name="产品属性"
    )
    ...
    weight = models.DecimalField(
        max_digits=10, decimal_places=2, null=True, blank=True, verbose_name="重量"
    )
    dimensions = models.CharField(
        max_length=50, blank=True, null=True, verbose_name="尺寸"
    )
